...
function setup()
{
	...
	document.getElementById('start').value = "PLAY GAME";
            ...
            if (!is_guessed[i])
            {
                ...
                for (var j=0; j<acceptable.length; j++)
                {
                    if (isAlphanumericMatch(acceptable[j], guess))
                    {
                        ...
                        document.getElementById(id).innerHTML = capitalizeFirstLetter(preferred);
...
function end()
{
    ...
    for (var i=0; i<is_guessed.length; i++)
    {
		...
		preferred = capitalizeFirstLetter(preferred);
